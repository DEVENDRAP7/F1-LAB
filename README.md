...
The car is no longer geometry this project lofts. It is an open-licence
concept model of a 2026 car — *F1 2026 concept (polygon model)* by
Qvist_designs, [CC-BY-4.0](http://creativecommons.org/licenses/by/4.0/) —
...
CPython 3.11 only), so the cut is a reviewable script rather than a
hand-edited binary, and rebuilding it is one command.

**Why a re-cut is needed at all.** The donor ships as eleven meshes named
`Object_0..Object_10`, and they are not components: each is roughly 115k
triangles of whatever fitted in one vertex buffer, so every one spans the
entire car. Rendering `Object_0` alone gives scattered fragments of front
wing, floor, a wheel and rear wing at once. Loaded as delivered, every
click on the rig would return the same meaningless fragment, which would
have taken the module's whole point with it. So the meshes are joined and
re-cut by where each polygon *is*, into the thirteen keys
`src/lib/aeroRigParts.js` knows about.

The regions are measured off the donor rather than guessed: a profile of
height and half-width every 200 mm puts the airbox apex at x 1900 (the
car's highest point), the axles near x −100 and x 3200 (where half-width
peaks at the 930 mm track), and the front wing assembly ending at x −420.
`npm run model:debug` colours every part differently, which is the only
practical way to see a segmentation rule that is wrong — it is how the
engine-bay flank was caught being called "nose", and the lower half of
each rear wing endplate being called "diffuser".

**What the cut cannot recover**, both flagged in the page copy: the donor
does not separate the front wing's movable elements from its fixed
mainplane, so the flap is taken as the upper-rear band; and wishbones
